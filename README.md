...
**力诺通用知识库 RAG (LinuoRAG)** 是一套专为**中大型集团企业、研发机构与管理部门**量身定制的私有化多模态 RAG (Retrieval-Augmented Generation，检索增强生成) 知识库与智能文书协同编排系统。
...
│  │  ┌──────────────────────────────────────────────────────────────┐  │  │
│  │  │               Go 核心网关 (Nexus-Gateway, Port 8001)         │  │  │
│  │  │  - 全局高并发接入与 JWT 校验  - L2 Redis 缓存拦截与异步回写   │  │  │
│  │  │  - 核心聊天流基于 Eino 框架图编排流程                        │  │  │
│  │  └──────────────────────────────┬───────────────────────────────┘  │  │
│  │                                 │ (NoRoute 泛路由反向代理)         │  │
│  │  ┌──────────────────────────────▼───────────────┐                  │  │
│  │  │           Python 算法后端 (Port 8002)         │                  │  │
│  │  │  - 复杂法律工作流流式生成   - 原生 Word 修订留痕与批注          │  │  │
...
### 混合架构核心技术设计：
1. **高性能 Go 网关与 Eino 编排**：由 Go 承载高并发网络连接（如长连接心跳、大纲编辑、文件上传与系统健康上报），单机协程（Goroutine）内存占用低，将高并发压力物理屏蔽。核心对话流引入字节跳动开源的 Eino 编排框架，支持复杂的条件路由与有向图多 Agent 协同，并提供毫秒级的级联取消支持，避免模型算力空转。
2. **反向代理与微服务分工**：Go 网关作为最前端流量控制层，直接利用高性能反向代理（Reverse Proxy）将重型、低频的法律事务流式分析（`/api/legal/workflow/stream`）及合同审查请求转发给 Python 微服务，充分结合 Go 的高并发吞吐能力与 Python 丰富的 NLP/AI 算法库生态。
...
| **富文本** | Tiptap 3 + Document Studio | 所见即所得的长文编辑器，支持前后文感知、Zustand 持久化防断电丢草稿。 |
| **算法服务** | FastAPI + Uvicorn + Python 3.12 | 异步处理法律工作流、ASR 转写、OCR 及 docx 批注，暴露纯净算法微服务。 |
...
系统内置高表现力、面向司法场景的检索增强生成（RAG）管道，采用六路并行检索融合机制：
...
```
 ...
 └─────┬─────┘         └─────┬─────┘          └─────┬─────┘         └─────┬─────┘
       ...
       └─────────────────────┼──────────────────────┼─────────────────────┘
                             │ asyncio.gather 并行融合
                    ┌────────┴────────┐
                    │ Reranker 精排   │ (BAAI/bge-reranker-v2-m3 二次打分)
                    └────────┬────────┘
                             │ (地名正字校对 + 结构化表格原子注入)
                    ┌────────┴────────┐
...
- **Cross-Project 双路检索与地名正字**：支持项目内部卷宗精查与外部国家法典库的跨库联合检索。从检索到的事实块中动态提取地名、面积、金额等专有名词进行强地名正字比对，防止模型产生同音字笔误。
...
LinuoRAG 创新性地引入了基于本地轻量级模型架构的 **多 Agent 协同工作流**，通过四智能体分工协同网络深度对齐真实的业务决策与分析场景，并利用 Go 网关与 Eino 编排框架实现极其灵敏的调度与超时控制：
...
3. **审查员二次 RAG 法条强校核**：在审查阶段，`Contrarian`（审查员）智能体会主动调用向量库检索工具，核实首稿中引用的具体法条是否与国家现行法典一致，自动查出“严重（法条虚构/废止）”、“一般（引用未说明）”与“建议（表述优化）”意见，从根本上根除大模型的编造幻觉。
...
- **全速学习模式 (Full Speed)**：后台同时启动文本向量化进程、Neo4j 图谱三元组构建以及 Louvain 社区摘要计算任务。大语言模型常驻 GPU 内存，后台多并发火力全开，以最高吞吐量处理新上传卷宗。
...
作为专为法律严苛场景量身打造的系统，LawRAG 在私有部署上构建了极其稳固的系统安全性设计：

- **物理与路由级数据隔离**：支持项目级的隔离权限校验，非本项目授权用户无法通过 API 获取该项目的知识卷宗。
...
│   ├── nexus-gateway               # Go 核心网关源码 (Port 8001)
│   │   ├── main.go                 # 网关入口与反向代理路由
│   │   ├── chat_handler.go         # L2 缓存拦截与 Eino 图调用控制
│   │   └── eino_graph.go           # 核心聊天流有向图 Eino 编排
│   ├── backend                     # Python FastAPI 算法微服务源码 (Port 8002)
...
* **`nexus-gateway` (Go 核心网关)**：监听 `8001` 端口，作为外部流量的唯一接入关隘。
* **`shengyao-backend` (Python 算法微服务)**：监听 `8002` 端口，接收网关转发的法律推理任务。
* **`shengyao-frontend` (React 前端服务)**：托管前端静态打包资源，承载客户端所有 UI 渲染与网络交互。
* **`shengyao-celery-fast` / `slow`**：后台高并发切片吞吐及慢速大计算量队列，守护 GPU 及内存防 OOM。
