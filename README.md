...
*   **4. 自动研判学习**：后台知识监控看板升级为“自动研判学习”，实时呈现三阶段公文推荐与大模型深度填报完成率，支持一键闭环全量推演。
...
## ⚡ 六、 智能助手极速直达与案卷自愈检索（0ms 规则旁路）

针对长问答大模型容易超时的痛点，系统打造了秒级直达与自愈检索体系：
*   **0ms 极速规则直达通道**：当提问命中“案件、受理、依据、立案、处罚、违法、事实、举报”等办案关键词时，自动跳过 LLM 预处理与改写排队，直接 0 毫秒启动检索，响应建立耗时由 30s+ 骤降至 **2.35 秒**。
*   **案件案卷自动无感加载**：用户未手动勾选文件时，进入案件自动扫描加载全量案卷素材与引用的国家标准/行政处罚法规，开箱即问。
*   **SQLite FTS5 全文索引与多词匹配**：为所有案卷构建 FTS5 全文倒排索引，结合关键词 OR 检索与模糊匹配兜底，彻底解决长句检索 0 命中缺陷。
*   **上下文智能限长与轻量化**：送入 LLM 的 context 限制在 3,800 字以内，`chat_num_ctx` 由 16K 优化至 8K，首字飞速流式吐字，彻底根治 329.5 秒超时假死。
