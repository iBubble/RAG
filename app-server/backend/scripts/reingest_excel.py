...
步骤:
  ...
  5. 用新的 _extract_xlsx 重新提取文本 → 切片 → 向量化入库
  6. 重新提取并注册表格
...
def main():
    ...
    print(f"  磁盘上找到 {len(disk_files)} 个 Excel 文件")
    for fid, (fp, fn, pid, rp) in disk_files.items():
        print(f"  📄 [{pid[:8]}] {fn} (id={fid[:8]})")

    # ══════════════════════════════════════════
    # Phase 2: 扫描 Qdrant，找出 Excel 相关的幽灵向量
    # ══════════════════════════════════════════
    print(f"\n{'=' * 60}")
    ...
    # ══════════════════════════════════════════
    # Phase 4: 重新入库所有 Excel 文件
    # ══════════════════════════════════════════
    print(f"\n{'=' * 60}")
    print(f"Phase 4: 重新入库 {len(disk_files)} 个 Excel 文件")
    print("=" * 60)
