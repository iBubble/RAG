...
### Removed & Deprecated
- **旧版平铺式「国家市场监督法律法规」知识库全链路彻底物理销毁**：
  - 从 SQLite 数据库 `projects` 及其所有关联子表中彻底物理抹除旧项目 `fc28c7fff7bb`；
  - 精准清空 Qdrant 向量数据库中所属的全部 **6,239 个稠密/稀疏切片与全文索引点**，即刻释放向量存储空间；
  - 彻底递归清理宿主机物理磁盘存储目录 `/Volumes/macData/GenRAG_Files/uploads/fc28c7fff7bb`，并同步清退 Redis L2 语义缓存与图谱关联缓存。

### Changed
- **全新「市监法规」高保真层级库全量接管**：
  - 由包含 20 个法制专篇分类、640 部现行有效法律法规规章（完整覆盖旧库 284 篇且多出 356 篇新法新规）的高保真层级知识库全面接管市场监管办案检索，彻底终结双库并存引发的冗余召回与版本冲突。

### Operations
- **全链路销毁与巡检工具沉淀**：
  - 沉淀 `scripts_destroy_project.py` 支持一键清空项目元数据、向量索引、物理文件与缓存；
  - 建立后台食品安全国家标准切片入库的自动化防卡死巡检体系。

## [4.6.0] - 2026-09-06

### Added
- **食品安全国家标准全库官方正本直采与图表高保真处理体系**：
  - 对接国家食品安全风险评估中心（CFSA）官方平台，100% 直采落盘 1,746 部官方正本（15 个国家标准大类、18,715 页、0% 损坏率），彻底清退第三方残卷；
  - 基于 PyMuPDF 核心引擎实现 2,568+ 个复杂限量表与微生物指标的原样 Markdown Table 坐标混排；
  - 保留 3,770+ 幅质谱/色谱图与检验流线图视觉锚点。
- **案卷多层级目录工作空间与多模态解析自愈管道**：
  - 全景树形目录支持任意节点新建目录与多级子目录，空目录保真常驻；
  - 前端剔除公共根目录剥离历史副作用，完整保留用户选定上传的文件夹名称与层级；
  - 多选文件批量跨目录移动，自适应纯文字极简按钮排版，物理文件与 Qdrant 索引元数据毫秒级联动；
  - 修复秒传去重死锁，对 `failed` 状态彻底穿透重试，消灭历史解析失败死锁。
